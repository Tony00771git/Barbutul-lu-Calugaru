...
export const FriendsTab: React.FC<FriendsTabProps> = ({
  ...
  useEffect(() => {
    ...
    // Local presence evaluation ticker (every 4s)
      ...
      {!user ? (
        ...
        <div className="w-full space-y-4">
          ...
          <div className="w-full bg-[#161009] border border-[#2d1e12] rounded-2xl p-3.5 space-y-2.5 shadow-md">
            ...
            {/* Search Result Card */}
            {searchResult && (() => {
              const resultStatus = formatLastSeenStatus(searchResult, language);
              return (
                <div className="p-3 rounded-xl bg-[#110c07] border border-[#ffd700]/70 flex items-center justify-between gap-3 shadow-md animate-fade-in">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="relative flex-shrink-0">
                      <div className="w-10 h-10 rounded-xl bg-[#1a120b] border border-[#ffd700] flex items-center justify-center overflow-hidden">
                        <AvatarDisplay avatarId={searchResult.avatarIcon || 'monk_drunk'} className="w-full h-full p-0.5" />
                      </div>
                      <span
                        className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#110c07] ${resultStatus.dotClass}`}
                        title={resultStatus.statusText}
                      />
                    </div>
                    <div className="min-w-0">
                      <div className="font-cinzel font-bold text-xs sm:text-sm text-[#ffd700] truncate flex items-center gap-1.5">
                        <span>{searchResult.displayName}</span>
                        <span
                          className={`w-2 h-2 rounded-full flex-shrink-0 ${resultStatus.dotClass}`}
                          title={resultStatus.statusText}
                        />
                      </div>
                      <div className="text-[10px] text-gray-300 font-barlow truncate flex items-center gap-1.5">
                        <span>Nv. {searchResult.currentLevel || 1} • {searchResult.currentTitle_ro || 'Frate Pelerin'}</span>
                        <span className={`px-1.5 py-0.2 rounded border text-[9px] ${resultStatus.badgeBgClass}`}>
          ...
          {friends.some((f) => isRoomActiveAndFresh(f.activeRoom) && f.activeRoom?.status === 'lobby') && (
            <div className="w-full bg-gradient-to-r from-[#201309] via-[#2a170b] to-[#201309] border-2 border-[#ffd700] rounded-2xl p-3.5 space-y-2.5 shadow-xl gold-glow animate-fade-in">
              ...
              <div className="space-y-2">
                {friends
                  .filter((f) => isRoomActiveAndFresh(f.activeRoom) && f.activeRoom?.status === 'lobby')
                  .map((friend) => {
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl bg-[#110c07] border transition-all flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 shadow ${
                        isLobby
                          ...
                          : statusInfo.isOnline
                          ? 'border-emerald-900/60 hover:border-emerald-500/50 bg-gradient-to-r from-[#10170e]/50 to-[#110c07]'
                          : 'border-[#2d1e12] hover:border-[#ffd700]/40 opacity-90 hover:opacity-100'
                      }`}
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        {/* Avatar with dynamic presence indicator (Green vs Grey) */}
                        <div className="relative flex-shrink-0">
                          <button
                            type="button"
                            onClick={() => setInspectFriendStats(friend)}
                            className={`w-10 h-10 rounded-xl bg-[#1a120b] border hover:border-[#ffd700] hover:scale-105 active:scale-95 transition-transform flex items-center justify-center overflow-hidden cursor-pointer shadow ${
                              statusInfo.isOnline ? 'border-emerald-500/60' : 'border-[#e8c84a]/40'
                            }`}
                            title={language === 'ro' ? 'Vezi Profilul și Statisticile Prietenului' : "View Friend's Profile & Stats"}
                          >
                            <AvatarDisplay avatarId={friend.avatarIcon || 'monk_drunk'} className="w-full h-full p-0.5" />
                          ...
                          <span
                            className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#110c07] ${statusInfo.dotClass}`}
                        ...
                        <div className="min-w-0 cursor-pointer" onClick={() => setInspectFriendStats(friend)}>
                          <div className="font-cinzel font-bold text-xs sm:text-sm text-[#f0ebe0] truncate flex items-center gap-1.5 hover:text-[#ffd700] transition-colors">
                            ...
                            {/* Inline visual dot */}
                            <span
                              className={`w-2 h-2 rounded-full flex-shrink-0 ${statusInfo.dotClass}`}
                              title={statusInfo.statusText}
                            />
                          </div>
                          <div className="text-[10px] text-amber-400/90 font-barlow flex items-center gap-1.5 flex-wrap mt-0.5">
                            {friend.shortId && <span className="font-mono text-amber-300/80">#{friend.shortId}</span>}
                            {friend.currentTitle_ro && (
                              <span className="text-gray-400 truncate">• {friend.currentTitle_ro}</span>
                            )}
                            {/* Dynamic Real-time Status Badge */}
                            <span className={`px-1.5 py-0.2 rounded border text-[9px] font-medium flex items-center gap-1 ${statusInfo.badgeBgClass}`}>
                              <span className={`w-1.5 h-1.5 rounded-full ${statusInfo.isOnline ? (isLobby ? 'bg-green-400 animate-pulse' : isInGame ? 'bg-amber-400' : 'bg-emerald-400') : 'bg-gray-500'}`} />
