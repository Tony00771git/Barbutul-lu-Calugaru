...
export const CloudAccountModal: React.FC<CloudAccountModalProps> = ({ isOpen, onClose }) => {
  const { user, cloudProfile, signInWithGoogle, signOut, updateCloudProfile, loading: authLoading } = useAuth();
              ...
              {!user ? (
                <div className="bg-[#140e08] border border-[#2e2216] rounded-2xl p-5 text-center space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-3 px-4 rounded-2xl bg-white text-gray-900 font-cinzel font-bold text-xs sm:text-sm hover:bg-gray-100 active:scale-95 shadow-xl transition-all flex items-center justify-center gap-2.5"
                  >
                  ...
                  <div className="text-[11px] text-gray-500 font-barlow">
                    * Nu este necesară parolă. Datele tale de joc sunt stocate securizat în cloud.
                  </div>
                </div>
              ) : (
