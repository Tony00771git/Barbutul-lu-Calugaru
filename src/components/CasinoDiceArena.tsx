...
export const CasinoDiceArena: React.FC<CasinoDiceArenaProps> = ({
  ...
  const getSkinStyles = () => {
    switch (skin) {
      case 'bone':
        return {
          bg: 'bg-gradient-to-br from-[#ffffff] via-[#f5efe4] to-[#dfd5c2]',
          border: 'border-[#dfd5c2]',
          dotColor: 'bg-[#1e1711]',
          glow: 'shadow-[0_12px_28px_rgba(0,0,0,0.6),0_0_15px_rgba(240,235,224,0.4)]',
          edgeBevel: 'ring-1 ring-white/60',
        };
      case 'wood':
        return {
          ...
          dotColor: 'bg-[#f8f5ee]',
          glow: 'shadow-[0_12px_28px_rgba(0,0,0,0.6),0_0_15px_rgba(166,106,56,0.5)]',
          edgeBevel: 'ring-1 ring-[#e2a26b]/40',
        };
      case 'gold':
      default:
        return {
          bg: 'bg-gradient-to-br from-[#fff2a3] via-[#e8c84a] to-[#a67c15]',
          border: 'border-[#ffe98a]',
          dotColor: 'bg-[#2b1704]',
          glow: 'shadow-[0_12px_30px_rgba(0,0,0,0.7),0_0_25px_rgba(232,200,74,0.6)]',
          edgeBevel: 'ring-2 ring-[#fff6b8]/70',
        };
    }
  };
  ...
  // Render 3x3 dot pattern for die face (1 to 6)
  const renderDots = (value: number) => {
    const patternMap: Record<number, boolean[]> = {
    ...
    const pattern = patternMap[value] || patternMap[1];

    return (
      <div className="grid grid-cols-3 grid-rows-3 w-full h-full p-2.5 sm:p-3 pointer-events-none">
        {pattern.map((hasDot, i) => (
          <div key={i} className="flex items-center justify-center w-full h-full">
            {hasDot ? (
              <div
                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full ${style.dotColor} shadow-[inset_0_1.5px_2px_rgba(0,0,0,0.8),0_1px_1px_rgba(255,255,255,0.4)]`}
              />
            ) : null}
          </div>
        ))}
    ...
    <div className="relative w-full rounded-2xl bg-gradient-to-b from-[#182618] via-[#0d180f] to-[#070e08] border-2 border-[#523b24] shadow-[inset_0_0_50px_rgba(0,0,0,0.85),0_8px_25px_rgba(0,0,0,0.6)] p-3 sm:p-5 flex flex-col items-center justify-center select-none min-h-[190px] sm:min-h-[220px]">
      ...
      <div className="relative z-10 flex items-center justify-center gap-8 sm:gap-14 my-2 w-full py-1">
        ...
        <div className="relative flex flex-col items-center justify-center">
          ...
          <div
            className={`w-20 h-20 sm:w-24 sm:h-24 min-w-[80px] min-h-[80px] sm:min-w-[96px] sm:min-h-[96px] rounded-2xl sm:rounded-3xl border-2 ${style.border} ${style.bg} ${style.glow} ${style.edgeBevel} relative flex items-center justify-center transform transition-transform ${
              isRolling ? 'animate-dice-drop-1' : 'hover:scale-105 duration-200'
            }`}
          >
            {renderDots(activeValues[0])}
          </div>
        </div>
